...
#define EXPECT_PATH_EQ(actual, expected_literal)                              \
        do                                                                    \
                {                                                             \
                        fs_path_t expected__                                  \
                            = fs_path_dupe ((expected_literal), NULL);        \
...
TEST (boost_path, lexically_normal)
{
        ...
        for (i = 0; i < sizeof (cases) / sizeof (cases[0]); ++i)
                {
                        fs_path_t out
                            = fs_path_lexically_normal (cases[i].in, &ec);
                        EXPECT_NO_EC (ec);
...
TEST (boost_path, lexically_relative)
{
        ...
        for (i = 0; i < sizeof (cases) / sizeof (cases[0]); ++i)
                {
                        fs_path_t out = fs_path_lexically_relative (
                            cases[i].path, cases[i].base, &ec);
                        EXPECT_NO_EC (ec);
                        EXPECT_PATH_EQ (out, cases[i].expected);
                        free (out);
                }
...
TEST (boost_path, lexically_proximate)
{
        ...
        out = fs_path_lexically_proximate (FS_PATH ("a/b/c"), FS_PATH ("x"),
                                           &ec);
        EXPECT_NO_EC (ec);
