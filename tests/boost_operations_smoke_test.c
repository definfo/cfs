...
#define BOOST_SMOKE_ROOT FS_PATH ("cfs-boost-operations-smoke")

#define EXPECT_NO_EC(ec) EXPECT_EQ ((ec).type, fs_error_type_none)

static void
create_file (const fs_cpath_t path)
{
        char *native_path;
        FILE *file;
...
TEST (boost_operations, directory_iterators)
{
        ...
        it = fs_recursive_directory_iterator (FS_PATH ("."), &ec);
        EXPECT_NO_EC (ec);
...
TEST (boost_operations, space_and_equivalent)
{
        ...
        info = fs_space (FS_PATH ("."), &ec);
        EXPECT_NO_EC (ec);
        EXPECT_TRUE (info.capacity >= info.free);
        EXPECT_TRUE (info.free >= info.available);

        EXPECT_TRUE (fs_equivalent (FS_PATH ("."), FS_PATH ("."), &ec));
        EXPECT_NO_EC (ec);
        EXPECT_FALSE (fs_equivalent (FS_PATH ("."), FS_PATH ("file"), &ec));
        EXPECT_NO_EC (ec);
}
...
main (void)
{
        ...
        create_file (FS_PATH ("file"));
        fs_create_directory (FS_PATH ("directory"), &ec);
        if (ec.type != fs_error_type_none)
                return EXIT_FAILURE;
        create_file (FS_PATH ("directory/nested-file"));
        ...
        fs_set_current_path (FS_PATH (".."), NULL);
        fs_remove_all (BOOST_SMOKE_ROOT, NULL);
        return result;
}
