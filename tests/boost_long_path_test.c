...
static fs_path_t
join3 (fs_cpath_t a, fs_cpath_t b, fs_cpath_t c)
{
        fs_error_code_t ec;
        fs_path_t p, q;
...
TEST (boost_long_path, no_escape)
{
        ...
        fs_create_directories (path,
                               &ec); /* whole tree, incl. >MAX_PATH depth */
        EXPECT_NO_EC (ec);
...
TEST (boost_long_path, absolute)
{
        ...
        root = fs_path_append (cwd, FS_PATH ("cfs-boost-longpath-absolute"),
                               &ec);
        free (cwd);
        comp1 = make_long_component ('x');
        comp2 = make_long_component ('y');
        ...
        fs_create_directories (
            path, &ec); /* long ABSOLUTE path -> CFS escapes directly */
        EXPECT_NO_EC (ec);
        EXPECT_TRUE (fs_exists (path, &ec) == FS_TRUE);
...
main (void)
{
        ...
        abs_root = fs_path_append (
            cwd, FS_PATH ("cfs-boost-longpath-absolute"), NULL);
        free (cwd);
