...
static void ops_create_file(const fs_cpath_t path, const char *contents)
{
        char *native_path;
        FILE *file;
...
static fs_path_t ops_join2(const fs_cpath_t a, const fs_cpath_t b)
{
        return fs_path_append(a, b, NULL);
}

static fs_path_t ops_join3(const fs_cpath_t a, const fs_cpath_t b, const fs_cpath_t c)
{
        fs_path_t out;
 ...
 * without making nonexistence itself an error (std::filesystem::status contract).
 */
