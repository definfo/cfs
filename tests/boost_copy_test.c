...
static void
copy_write_file (const fs_cpath_t path, const char *contents)
{
        char *native_path;
        FILE *file;
...
static void
copy_expect_file_contents (const fs_cpath_t path, const char *expected,
                           int *ret)
{
        char buf[64];
        char *native_path;
...
static fs_path_t
copy_join2 (const fs_cpath_t a, const fs_cpath_t b)
{
        return fs_path_append (a, b, NULL);
}

static fs_path_t
copy_join3 (const fs_cpath_t a, const fs_cpath_t b, const fs_cpath_t c)
{
        fs_path_t out;
