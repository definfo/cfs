        ...
        const append_case_t cases[]
            = { { FS_PATH (""), FS_PATH (""), FS_PATH (""), FS_TRUE },
                ...
                { FS_PATH ("foo"), FS_PATH ("bar"), FS_PATH ("foo/bar"),
                  FS_TRUE },
                /* Existing separators are preserved; only inserted separators
                   are preferred. */
        ...
        for (i = 0; i < sizeof (cases) / sizeof (cases[0]); ++i)
                {
                        ...
                        out = fs_path_append (cases[i].path, cases[i].arg,
                                              &ec);
                        EXPECT_NO_EC (ec);
                        if (cases[i].expected_preferred)
                                EXPECT_PATH_EQ_PREFERRED (out,
                                                          cases[i].expected);
                        else
                                EXPECT_PATH_EQ_LITERAL (out,
                                                        cases[i].expected);
                        free (out);
                        ...
                        EXPECT_NO_EC (ec);
                        if (cases[i].expected_preferred)
                                EXPECT_PATH_EQ_PREFERRED (in_place,
                                                          cases[i].expected);
                        else
                                EXPECT_PATH_EQ_LITERAL (in_place,
                                                        cases[i].expected);
                        free (in_place);
                }
}
        ...
        for (i = 0; i < sizeof (cases) / sizeof (cases[0]); ++i)
                {
                        ...
                        out = fs_path_concat (cases[i].path, cases[i].arg,
                                              &ec);
                        EXPECT_NO_EC (ec);
