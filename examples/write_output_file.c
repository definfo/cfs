 ...
 *
 * Build: cc -Iinclude examples/write_output_file.c -o write_output_file
 */
...
int main(void)
{
        ...
        backup_path = fs_path_append(FS_PATH("output"), FS_PATH("report.bak"), &ec);

        if (fs_exists(out, &ec) && !append)        /* snapshot before overwrite */
                fs_copy_file_opt(out, backup_path, fs_copy_options_overwrite_existing, &ec);

        if (fs_exists(out, &ec) && !append)        /* overwrite == remove, then write */
                fs_remove(out, &ec);
