...
static fs_bool_t
write_buffer (fs_cpath_t path, const char *buf, fs_umax_t len,
              fs_error_code_t *ec)
{
        io_file_t f;
        ...
        while (offset < len)
                {
                        n = io_file_write (f, buf + (size_t)offset,
                                           len - offset, ec);
...
static fs_bool_t
contents_equal (fs_cpath_t a_path, fs_cpath_t b_path, fs_error_code_t *ec)
{
        io_file_t a, b;
...
main (int argc, char *argv[])
{
        ...
        /* ensure parent exists; temp beside the target (same fs -> atomic
         * rename) */
