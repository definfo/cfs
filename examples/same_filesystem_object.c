...
main (int argc, char *argv[])
{
        ...
        /* same filesystem object (inode/device), like
         * realpath(a)+realpath(b)+strcmp */
        if (fs_equivalent (a, b, &ec))
                printf ("same object\n");

        /* fs_status follows a symlink to its target; fs_symlink_status reports
         * the link itself. Both return an fs_file_status_t for type/perms
         * queries. */
