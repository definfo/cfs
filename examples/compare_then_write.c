 ...
 *
 * Build: cc -Iinclude examples/compare_then_write.c -o compare_then_write
 * Run:   ./compare_then_write <target> <content>
 */
...
static fs_bool_t contents_equal(const char *buf, fs_umax_t len, fs_cpath_t path,
                                fs_error_code_t *ec)
{
        io_file_t f;
        char         chunk[4096];
        ...
        for (;;) {
                ...
                if (offset > len || n > len - offset ||
                    memcmp(buf + (size_t)offset, chunk, (size_t)n) != 0)
                        break;
                offset += n;
        }
...
int main(int argc, char *argv[])
{
        ...
        if (exists) {
                ...
                if (ec.type != fs_error_type_none) {
                        fprintf(stderr, "compare(target) failed: %s\n", ec.msg ? ec.msg : "");
                        free(target);
                        return 1;
                }
                if (equal) {
